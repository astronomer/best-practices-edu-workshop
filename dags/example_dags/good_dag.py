...
BASE_DATA_PATH = os.getenv("BASE_DATA_PATH", "include/data/")
IMPORT_PATH_CUSTOMER_FEEDBACK = f"{BASE_DATA_PATH}/customer_feedback.csv"
IMPORT_CUSTOMER_DATA = f"{BASE_DATA_PATH}/customer_data.csv"
IMPORT_PATH_SALES_REPORTS = f"{BASE_DATA_PATH}/sales_reports.csv"
...
@dag(
    ...
    max_consecutive_failed_dag_runs=5,  # max number of failed DAG runs before the DAG auto-pauses (experimental, 2.9)
    default_args={
        "owner": "Avery",
        "retries": 3,
        "retry_delay": 5,
...
def good_dag():
    ...
    @task(map_index_template="{{ my_map_index }}")
    def analyze_sentiment(comment: str) -> float:
        ...
        context["my_map_index"] = (
            "Feedback: " + comment + " Sentiment: " + str(sentiment["sentiment_label"])
        )
...
# optional: use dag.test() to test the DAG
