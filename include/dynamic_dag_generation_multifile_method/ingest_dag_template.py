"""
## Dynamically generated DAG for ingestion of source_name_to_replace
...
@dag(
    dag_id=dag_id_to_replace,  # These undefined variables will be templated in by the script
    start_date=parse(start_date_to_replace),
    schedule=schedule_to_replace,
    catchup=catchup_to_replace,
    tags=tags_to_replace,
    default_args={
        "owner": dag_owner_to_replace,
        "retries": task_retries_to_replace,
        "retry_delay": duration(seconds=5),
    },
    description=f"Ingest data from source_name_to_replace",
    doc_md=__doc__,
)
def ingest_dag():
    @task.sensor(
        task_id=f"wait_for_new_files_source_name_to_replace",
    ...
    @task(task_id=f"extract_source_name_to_replace")
    def extract(
    ...
    @task(task_id=f"load_source_name_to_replace")
    def load(
        source_file: ObjectStoragePath,
        ...
        load_file_loc = ObjectStoragePath(
            f"{base_path_load}source_name_to_replace/{file_name}",
    ...
    update_dataset_obj = EmptyOperator(
        task_id=f"update_dataset_source_name_to_replace",
        outlets=[Dataset(dataset_uri_to_replace)]
    )

    # Calling TaskFlow tasks, inferring dependencies
    source_files = wait_for_new_files(
        base_path_ingest_to_replace,
        path_extension_to_replace,
        conn_id_ingest_to_replace
    )

    # Dynamically map the extract, transform and load tasks over the list of new file locations
    extract_obj = extract.partial(
        base_path_intermediate=base_path_intermediate_to_replace,
        conn_id_intermediate=conn_id_intermediate_to_replace, 
    ...
    load_obj = load.partial(
        ...
        conn_id_load=conn_id_load_to_replace,
